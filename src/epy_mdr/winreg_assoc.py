...
``Software\epy_mdr`` and ``Software\RegisteredApplications``), so
...
def _launcher_path() -> str:
    """Locate the installed ``epy_mdr`` launcher executable.

    Falls back to ``python -m epy_mdr`` if no console script was
...
def register(make_default: bool = False) -> list[str]:
    """Register epy_mdr for ``.md`` / ``.markdown`` / ``.qmd`` in HKCU.
