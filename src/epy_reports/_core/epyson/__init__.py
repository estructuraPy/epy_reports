...
# ---------------------------------------------------------------- load
#
# All raw ``.epyson`` I/O (bundled assets and user-saved themes) is
# delegated to ``epy_reports._config._loader`` -- this module only builds
# ``Theme``/CSS/Qt objects from the data it returns.
...
def _load_palettes() -> dict[str, dict[str, Any]]:
    ...
    Raises:
        FileNotFoundError: If ``colors.epyson`` is missing from the
            ...
            ``epy_reports._config._loader.load_color_palettes``).
    """
...
def save_user_theme(payload: dict[str, Any]) -> str:
    ...
    _loader.write_user_theme_json(directory / f"{layout_id}.epyson", payload)
    return layout_id
